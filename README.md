...
5. **自動更新が有効化**: `.github/workflows/update.yml` が毎週水曜の配信後にRSSとAWQランキングを更新し、生成物をコミットします。基本ノータッチでOK

## 日常の運用

### 新エピソード → 何もしなくてOK
毎週水曜23:30(JST)にGitHub Actionsが自動実行され、タイトル・配信日・概要文・チャプター・登場ゲームタイトルとAWQランキングが自動反映されます。
...
### エピソード別の配信リンク(自動)
各回ページの配信ボタン(Spotify/Apple/LISTEN/YouTube)は**その回のエピソードページ**に
直接飛びます。毎週の自動更新で `fetch_links.py` が取得します:
- **Spotify**: ボタンは `open.spotify.com` のエピソードURL(creators番組ページから全回分を
  自動対応付け)。ページ内の埋め込みプレイヤーも `open.spotify.com` の公式埋め込み
  (取得できていない回だけ従来のcreators埋め込みにフォールバック)
- **Apple**: iTunes Lookup API / **LISTEN**: LISTENのRSS(どちらもguidで確実に対応付け)
- **YouTube**: site.json の `youtube_playlist`(Podcastプレイリスト)から「第N回」で対応付け。
  **新しい回をプレイリストに登録するだけで自動リンク**されます。100件超は継続APIで
  自動追加取得。プレイリストに無い回(第0回等)は `"links": {"youtube": "..."}` の手動設定が生きます
- **Amazon Music**: 自動取得の手段がないため、必要なら `"links": {"amazon": "..."}` を手動設定
- 手動設定したリンク(amazon等)は自動更新で消えません
...
### サイト内プレイヤー(各回ページで直接再生)
各エピソードページには**サイト独自のプレイヤー**が付いています。RSSの音声ファイル(enclosure)を
HTML5 audioで直接再生する方式で、Spotifyの埋め込みプレイヤーは使っていません。

サイト内のページ移動・戻る・進むでは同じaudio要素が再生を続けます。再生ページを離れると、
画面下部の常駐プレイヤーから再生/一時停止・シーク・スキップ・速度変更ができます。
別の回へ移動しただけでは再生内容は変わらず、その回の再生ボタンやチャプターを押すと切り替わります。
実装・保守方法とブラウザ検証結果は [常駐プレイヤーの説明](docs/persistent-player.md) を参照してください。

- **チャプターを押すとその話題の頭から再生**。再生中のチャプターはハイライトで追従します
- 倍速(1.0〜2.0x・端末に記憶)・±スキップ・**前回のつづきから再生**(端末に記憶)対応
- **`?t=秒` 付きURLで頭出し共有**ができます(例: `episodes/110.html?t=941` は15:41から)。
  プレイヤーの「🔗 この位置のリンク」ボタンでいま聴いている位置のURLをコピーできます
- 滝壺データベースの各タイトルページでは、チャプター一致した回に **▶時刻チップ** が付き、
  押すとその話題の頭から再生されます
- データは `episodes.json` の `audio`(MP3のURL)と `duration`(秒)。毎週の自動更新で
  新着回にも自動で入ります(`locked` の回でもこの2つだけは常に最新へ更新)
- ⚠ このプレイヤーでの再生は**Spotifyアプリの再生数にはカウントされません**
  (Spotify for PodcastersのCDNから直接取得するため、統計上は他のポッドキャストアプリと
  同じ「RSS経由」の扱いになります)
...
1. 最新の`シーズンN`シートのA〜C列へ「回・名前・ポイント」を追加する
...
#### シーズンが切り替わるとき（シーズン4の例）

得点シートは`シーズン1`、`シーズン2`……という名前から自動検出されます。
最も番号の大きいシートが「集計中」、それ以前は「最終結果」になるため、コードの変更は不要です。

1. シーズン3の最終採点を入力し、集計が正しいことを確認する
2. 最新の得点シートを複製し、シート名を正確に`シーズン4`へ変更する
3. 複製したシートの旧得点明細（A2:D）を消し、F:Iの名簿・集計欄も新シーズン用に整理する
   - F列のX URLとG列の名前は、過去シーズンに登録済みなら新シートへ再入力しなくてもサイト側で引き継がれる
   - H:Iはシート上の確認用。サイトの順位はA:Cの明細から再集計する
4. シーズン4の開始回が第120回なら、得点行とは別に **A2へ`120`、D2へ`シーズン開始`** と入力する（B2/C2は空欄）
5. GitHub Actionsを手動実行するか、次回の定期実行を待つ

`シーズン開始`行は得点に含まれず、表示上の区切りだけに使われます。前シーズン最終回が未採点でも
クイズ画像を正しいシーズンへ分けられるため、新シーズンでは必ず入れてください。
シート番号は欠番にせず、`シーズン3`の次は`シーズン4`のように連番にします。
...
(GitHub上で `Actions → RSS・AWQから自動更新 → Run workflow` を押しても同じことが手動でできます)
...
| `description` | RSSの概要全文。**改行はそのまま表示されます**(`\n` を入れればページでも改行)。ページでは最初の`■`までを常時表示し、残りは展開して確認できます |
...
- なまえと最新エピソード番号をシードにした抽選なので、同じ最新回では同じ表記のなまえから同じ結果が出ます。新しい回が追加されると結果は再抽選されます
- よく話題に出るタイトルほど出やすくなります
- 結果に変なワードが出る場合は `data/episodes.json` の該当回の `games` からそのワードを消せば消えます
  (「／」を含む行・「【」で始まる行・「※」を含む行は自動で除外されます)
- 画面表示や抽選ロジックを変えたいときは `assets/js/shindan.js` を編集
...
- 各回への「語られ度」は4段階で自動判定:
  ...
  (絵文字・バッジ文言は `build.py` の `LEVEL_BADGE`/`LEVEL_MARK`、
  索引カードの背景色は `style.css` の `.gm-item.lv3` / `.gm-item.s3` で調整)

**⭐メインの判定(概要欄の新フォーマット)**:
概要欄の見出しを `■主な登場ゲームタイトル（★=メインで語ったタイトル）` にし、
メインのタイトル行の先頭に `★` を付けると、RSS取り込みで自動的にメイン扱いになります。
- **★が正**: ★の付いた行だけがメインになります(★ゼロ=メインなしの明示)
- 全回が★フォーマット化済みのため、自動推定は廃止しました。ゲームが載っているのに
  featured_games が無い回(=★凡例の書き忘れの可能性)はビルド時に警告が出ます
