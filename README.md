...
- 各回への「語られ度」は自動判定: エピソードタイトルに含まれる=🎙メイン、チャプターに含まれる=📑チャプター、それ以外=💬トーク内
- **自動判定が外れた回の手動修正**: `data/episodes.json` の該当回に1行足す:
  ```json
  "featured_games": ["風来のシレン"],
  ```
  → その回がそのタイトルの「🎙メインで語られた回」として索引に載ります。
  概要欄に「○○シリーズ」としか書いていない回を個別タイトルのページに出したいときもこれでOK。
  このフィールドはRSS自動更新で消えないので、`"locked": true` は不要です
